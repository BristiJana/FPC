...
const [highlightedDates, setHighlightedDates] = useState([]);
  ...
  const highlightedDatesList = soildata.filter(data => data.png.lswi !== 'None');
...
const formattedDate = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
    ...
    imageUrlByDate[formattedDate] = data.png.lswi; 
  });
...
const [markedDates, setMarkedDates] = useState({}); // Store marked dates here
  ...
  useEffect(() => {
    ...
    soildata.forEach((data) => {
      if (data.png.lswi !== 'None') {
       
        console.log(data.png.lswi)
...
console.log(formattedDate)
  ...
  //       const url = value.zonalStats.lswi;
      ...
      datasets: [
        {
          ...
          //   console.log(value.zonalStats.lswi);
          //   const dataFromHandleGet = handleget(value.zonalStats.lswi);
          //   console.log("hey")
...
const todayImageData = soildata.find(data => {
  const date = new Date(data.Date);
  const formattedDate = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
  return formattedDate === todayFormatted && data.png.lswi !== 'None';
});

// Assign the appropriate srcimg value
const srcimg = todayImageData ? todayImageData.png.lswi : 'None';
    console.log(srcimg)
